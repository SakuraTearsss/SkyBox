...
set choice=
if "%choice_default%"=="" (
    set /p choice="输入序号按Enter继续: "
) else (
 set /p choice="输入序号按Enter继续(默认:%choice_default%): "
)
if not "%var2%"=="" (if not "%choice_default%"=="" (if "%choice%"=="" set choice=%choice_default%))
echo.%choice% | find "[" 1>nul 2>nul && ECHO 输入错误, 输入的内容不应带有[]符号. 请重新输入.  && goto COMMON-5
echo.%choice% | find "]" 1>nul 2>nul && ECHO 输入错误, 输入的内容不应带有[]符号. 请重新输入.  && goto COMMON-5
echo.%choice% | find "#" 1>nul 2>nul && ECHO 输入错误, 输入的内容不应带有#符号. 请重新输入.  && goto COMMON-5
...
if not "%var2%"=="" echo.%var2% | find "[%choice%]" 1>nul 2>nul || ECHO 输入错误, 输入的选项不在%var2%中. 请重新输入.  && goto COMMON-5
