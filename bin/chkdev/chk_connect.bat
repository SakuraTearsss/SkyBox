...
set chk_connect_try_max=1
::复查等待时间：如果使用rechk，则第一次检查之后等待几秒再进行第二次检查
set chk_connect_rechk_wait=0
...
::开始
:CHK-CONNECT-1
set chk_connect_try_times=0& set chk_connect_devnum=0& set chk_connect_devstate=blank& adb devices 1>nul
:CHK-CONNECT-2
...
if "%chk_connect_devnum%"=="0" (
    if %chk_connect_try_times% GEQ %chk_connect_try_max% ECHO.本次未检测到目标设备，请连接设备后按任意键重新检测... & pause>nul & goto CHK-CONNECT-1
...
if "%chk_connect_rechk%"=="y" (
    ...
    TIMEOUT /T %chk_connect_rechk_wait% /NOBREAK>nul & goto CHK-CONNECT-1)
::无需复查，涉及端口则检查端口号
